...
import type { Quote, LineItem } from '@/hooks/useClientBilling';

interface Props {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: Partial<Quote> & { line_items: LineItem[] }) => Promise<void>;
...
export const ClientQuoteForm = ({ isOpen, onClose, onSave, existingQuote, existingLineItems, defaultTaxRate = 0, defaultNotes = '' }: Props) => {
  ...
  useEffect(() => {
    if (existingQuote) {
      ...
      setIssueDate(existingQuote.issue_date);
      setValidUntil(existingQuote.valid_until || '');
      setNotes(existingQuote.notes || '');
      setTaxRate(existingQuote.subtotal > 0 ? (Number(existingQuote.tax_amount) / Number(existingQuote.subtotal)) * 100 : defaultTaxRate);
    } else {
      setStatus('draft');
      setIssueDate(new Date().toISOString().split('T')[0]);
      const exp = new Date(); exp.setDate(exp.getDate() + 30);
      setValidUntil(exp.toISOString().split('T')[0]);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{existingQuote ? 'Edit Quote' : 'New Quote'}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={handleSave} disabled={saving}>{saving ? 'Saving…' : existingQuote ? 'Update Quote' : 'Create Quote'}</Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
